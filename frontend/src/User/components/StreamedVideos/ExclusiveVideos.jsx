...
import {useGetExclusiveVideosMutation } from '../../slices/api_slices/videoStreamApiSlice'
import { useCheckUserWalletStatusMutation } from '../../slices/api_slices/userMonetizationApiSlice'
...
const ExclusiveVideos = ()=>{
  ...
  const handleMouseLeave = () => {
    ...
    const [getStreams] = useGetExclusiveVideosMutation();
    ...
    const VideoStreams = ()=>{
          return(
            loading && isLoading ?  <div className="w-full flex justify-center h-full">
            <div className="py-52">
        ...
        >
          <Tooltip
           content={
            `${video.coin_for_watch} to watch the video`
          }
          placement={
            "bottom-end"
          }
          animate={{
            mount: { scale: 1, y: -150 ,x:-100},
            unmount: { scale: 1,  y: -150 ,x:-100},
          }}
          >
    ...
    return(
       ...
       <div className="m-4">
         <div className="card_header">
          <h1 className="card_title">Exclusive Videos</h1>
         </div>
         {urlVideos.length > 0
        ? 
        ...
        <div className="flex justify-center">
           <h1 className="font-semibold text-2xl m-12">No Exclusive Videos</h1>
        </div>
       }
        </div>
        </section>
